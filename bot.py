        ...
        try:
            ...
            for survey_id, data in surveys_data.items():
                try:
                    ...
                    if not all(key in data for key in ["name", "description", "questions"]):
                        logger.error(f"Отсутствуют обязательные поля в данных опроса {survey_id}")
                        continue
            ...
            for line in content.split('\n'):
                ...
                if "🆔 ID:" in line:
                    ...
                    try:
                        ...
                        current_survey = current_id.split("_")[1]
                ...
                elif "✅ Опрос завершён" in line and current_user_id:
                    is_completed = True
...
async def process_view_survey_users(callback: CallbackQuery) -> None:
    ...
    try:
        ...
        if not users:
            ...
            await callback.message.delete()
            await callback.message.answer(
                "Нет пользователей, прошедших этот опрос.",
                reply_markup=keyboard
            )
            await callback.answer()
            return
        ...
        buttons.append([
            InlineKeyboardButton(text="⬅️ Вернуться к списку опросов", callback_data="view_user_answers")
        ])
...
async def process_user_answers(callback: CallbackQuery) -> None:
    ...
    try:
        ...
        while i < len(lines):
            ...
            if "👤 Пользователь: @" in line:
                ...
                if i < len(lines):
                    ...
                    if "🆔 ID:" in id_line:
                        try:
                            parts = id_line.split("ID: ")[1].split("_")
                            current_user_id = parts[0]
                            current_survey = parts[1]
        ...
        if not found_user or not user_answers:
            keyboard = InlineKeyboardMarkup(
                inline_keyboard=[[
                    InlineKeyboardButton(text="⬅️ Назад к списку пользователей", callback_data=f"view_answers_survey_{survey_id}")
                ]]
        ...
        for i, (question, answer) in enumerate(user_answers, 1):
